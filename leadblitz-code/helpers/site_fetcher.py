...
def fetch_site_safely(url: str, timeout: int = 15, max_retries: int = 3) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Safety check: detect garbled/binary responses and retry without compression
            if response.status_code in [200, 202] and response.text:
                ...
                # Look for signs of binary data (control characters, non-printable chars)
                suspicious_chars = sum(1 for c in text_sample if ord(c) < 32 and c not in '\n\r\t')
                ...
                if suspicious_chars > 20:  # Too many control characters, likely binary
                    ...
                    try:
                        ...
                        if clean_response.status_code in [200, 202] and clean_response.text:
                            result["html"] = clean_response.text
                            result["status"] = clean_response.status_code
                            result["final_url"] = clean_response.url
                            result["errors"] = ["Fixed garbled response by removing compression"]
                            return result
                    except:
                        pass  # Fall through to original response processing
